...
acs_nces_data |> 
  ...
  filter(year != 2010) |> 
  ggplot(aes(x = gentrifying, 
             y = economic_disadvantaged_enrollment)) + 
  geom_boxplot()
...
acs_nces_data |> 
  ...
  filter(year != 2010) |> 
  ggplot(aes(x = gentrifying, 
             y = grade_11_english_percent_ready)) + 
  geom_boxplot()
...
acs_nces_data |> 
  ...
  filter(year != 2010) |> 
  ggplot(aes(x = gentrifying, 
             y = grade_11_all_subject_percent_ready)) + 
  geom_boxplot()
...
acs_nces_data |> 
  ...
  select(year, rent_delta, value_delta, edu_level_delta, gentrifying, everything()) |> 
  filter(year != 2010) |> 
  pivot_longer(cols = contains('enrollment') & !starts_with('grade') & !starts_with('kinder'), 
               names_to = 'enrollment_group', 
               values_to = 'enrollment_number') |> 
  select(enrollment_group, enrollment_number, everything()) |> 
