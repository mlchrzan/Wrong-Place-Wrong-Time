...
#Detroit Parcel Data
det_parcel <- read_csv("Detroit Housing Data/Parcels.csv") |> clean_names()
...
housing_built <- det_parcel |> 
  filter(year_built >= 2009 & year_built < 2019, 
        str_detect(property_class_desc, 'RESIDENTIAL')) |> 
  group_by(year_built) |> 
...
all_data |> 
  mutate(rent_delta = gross_rent_median_dollars - lag(gross_rent_median_dollars), 
         value_delta = value_median_dollars - lag(value_median_dollars), 
         edu_level_delta = percent_educational_attainment_graduate_or_professional_degree - lag(percent_educational_attainment_graduate_or_professional_degree), 
         housing_delta = housing_built - lag(housing_built)) |> 

  mutate(gentrifying = if_else((rent_delta > 0 | value_delta > 0) & edu_level_delta > 0.1 & housing_delta > 0, TRUE, FALSE)) |> 
  select(year, rent_delta, value_delta, edu_level_delta, housing_delta, gentrifying, everything())
```

```{r verify-gentrification-OLD}
#VARIABLES TO CONSIDER
#college educated increase = percent_educational_attainment_percent_bachelors_degree_or_higher (what others might we consider?)
#housing price increase = gross_rent_median_dollars (OR value_median_dollars? Should it be both?), could consider gross_rent_1500_or_more
#construction increase = structures_built (NEED BETTER DATA BEFORE ADDING TO QUALIFIERS)
...
acs_nces_data |> 
  ...
  mutate(gentrifying = if_else((rent_delta > 0 | value_delta > 0) & edu_level_delta > 0.5, TRUE, FALSE)) |> 
  select(year, rent_delta, value_delta, edu_level_delta, gentrifying, everything())


acs_nces_data |> 
  mutate(rent_delta = gross_rent_median_dollars - lag(gross_rent_median_dollars), 
         value_delta = value_median_dollars - lag(value_median_dollars), 
         edu_level_delta = percent_educational_attainment_percent_high_school_graduate_or_higher - lag(percent_educational_attainment_percent_high_school_graduate_or_higher)) |> 

  mutate(gentrifying = if_else((rent_delta > 0 | value_delta > 0) & edu_level_delta > 0, TRUE, FALSE)) |> 
  select(year, rent_delta, value_delta, edu_level_delta, gentrifying, everything())

#--------------------------------------------------------------------------------------------------------
#THIS SETUP FOR GENTRIFYING GIVES LATER YEARS AS GENTRIFYING. TESTING SOME VIZ
acs_nces_data |> 
  mutate(rent_delta = gross_rent_median_dollars - lag(gross_rent_median_dollars), 
         value_delta = value_median_dollars - lag(value_median_dollars), 
         edu_level_delta = percent_educational_attainment_graduate_or_professional_degree - lag(percent_educational_attainment_graduate_or_professional_degree)) |> 

  mutate(gentrifying = if_else((rent_delta > 0 | value_delta > 0) & edu_level_delta > 0.2, TRUE, FALSE)) |> 
  select(year, rent_delta, value_delta, edu_level_delta, gentrifying, everything()) |> 
  filter(year != 2010) |> 
...
acs_nces_data |> 
  ...
  ggplot(aes(x = year, 
             y = pct_total_enrollment)) + 
  geom_line(aes(color = gentrifying)) +
